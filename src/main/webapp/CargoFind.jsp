...
<div class="layout">
    ...
    <%--    <form action="/CargoListServlet" method="post">--%>
    <h1 class="pageHead">港口货物管理系统><span class="subPageHead">货物查询</span></h1>
<div class="tables">
    <form method="post" action="CargoFindServlet" class="form-inline">
        <div class="tables">
            <label for="id">货物编号</label>
            <input type="text" value="${condition.id[0]}" name="id" id="id" class="form-control">

            <label for="name">货物名称</label>
            <input type="text" value="${condition.name[0]}" name="name" id="name" class="form-control">
    ...
    <div class="maintable">
        <table class="cargotable">
            <tr class="table1"><td class="tablth">货物编号</td><td class="tablth tablehCargoName">货物名称()</td><td class="tablth">货物重量(单位:t)</td><td class="tablth">货物种类</td><td class="tablth">所在港口</td><td class="tablth">货物所属</td><td class="tablth">入库时间</td><td class="tablth">货物状态</td></tr>
            <%--              <tr class="table2">--%>
    ...
    <nav aria-label="Page navigation" class="pageBar ">
        <ul class="pagination">
            <%--最前页        --%>
            <li><a href="/PortCargoSystem_war_exploded/CargoFindServlet?currentPage=1&rows=10&id=${condition.id[0]}&name=${condition.name[0]}"><<</a></li>
            <%--向前翻页功能--%>
            <c:if test="${pc.currentPage == 1}">
                ...
                <c:forEach begin="1" end="${(pc.totalPage < 10) ? (pc.totalPage) : (10)}" var="i">
                    <c:if test="${pc.currentPage==i}">
                        <li class="active"><a href="/PortCargoSystem_war_exploded/CargoFindServlet?currentPage=${i}&rows=10&id=${condition.id[0]}&name=${condition.name[0]}">${i}</a></li>
                    </c:if>

                    <c:if test="${pc.currentPage!=i}">
                        <li><a href="/PortCargoSystem_war_exploded/CargoFindServlet?currentPage=${i}&rows=10&id=${condition.id[0]}&name=${condition.name[0]}">${i}</a></li>
                    </c:if>
                </c:forEach>
            </c:if>
            <%----%>
            <c:if test="${pc.currentPage != 1 }">
                <li><a href="/PortCargoSystem_war_exploded/CargoFindServlet?currentPage=${pc.currentPage-1}&rows=10&id=${condition.id[0]}&name=${condition.name[0]}" aria-label="Previous"><span aria-hidden="true"><</span></a></li>
